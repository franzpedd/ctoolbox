...
static inline bool bit_test(uint32_t* bits, uint32_t idx) {
    return bits[BIT_WORD(idx)] & BIT_MASK(idx);
}

static inline void bit_set(uint32_t* bits, uint32_t idx) {
    bits[BIT_WORD(idx)] |= BIT_MASK(idx);
}

static inline void bit_clear(uint32_t* bits, uint32_t idx) {
    bits[BIT_WORD(idx)] &= ~BIT_MASK(idx);
}
...
CTOOLBOX_API idgen* idgen_create(uint32_t start_id, uint32_t max_id)
{
    return idgen_create_memfuncs(start_id, max_id, &CTOOLBOX_DEFAULT_MEMFUNCS);
}

CTOOLBOX_API idgen* idgen_create_memfuncs(uint32_t start_id, uint32_t max_id, const ctoolbox_memfuncs* memfuncs)
{
    const ctoolbox_memfuncs* actual_memfuncs = memfuncs ? memfuncs : &CTOOLBOX_DEFAULT_MEMFUNCS;
    if (start_id >= max_id) return NULL;
    ...
    uint32_t range = max_id - start_id;
    gen->bitset_size = (range + 31u) / 32u; // round up to 32 bits
...
CTOOLBOX_API uint32_t idgen_next(idgen* gen)
{
    ...
    for (uint32_t i = 0; i < range; i++) {
        ...
        if (!bit_test(gen->used_bits, idx)) {
            bit_set(gen->used_bits, idx);
            gen->count++;
...
CTOOLBOX_API bool idgen_register(idgen* gen, uint32_t id)
{
    ...
    if (bit_test(gen->used_bits, idx)) return false;
...
CTOOLBOX_API bool idgen_is_registered(idgen* gen, uint32_t id)
{
    if (!gen || id < gen->start_id || id >= gen->max_id) return false;
    uint32_t idx = BIT_INDEX(id, gen->start_id);
    return bit_test(gen->used_bits, idx);
}
